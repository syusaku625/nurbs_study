...
int main()
{
    ...
    ifs.close();
    vector<double> knot_vector = {0,0,0,0,1,2,3,4,5,6,6,6,6};
    int order = 3;
    int numOfk=knot_vector.size()-order-1;
    vector<double> insert_knot = {1.0, 1.0, 2.0, 2.0, 3.0, 3.0, 4.0, 4.0, 5.0, 5.0};
    ...
    for(double i=0.0; i<=6.0; i+=0.001){
        double sum_X =0.0;
        double sum_Y = 0.0;
        for(int j=0; j<final_operator.size(); j++){    
            for(int k=0; k<final_operator[0].size(); k++){
                sum_X += final_operator[j][k] * p_order_N(knot_vector, j+1, order, i)*control_point[k][0];
                sum_Y += final_operator[j][k] * p_order_N(knot_vector, j+1, order, i)*control_point[k][1];
            }
        }
    ...
    vector<vector<double>> extraction_control_point(final_operator.size(), vector<double>(2));
    for(int i=0; i<final_operator.size(); i++){
        for(int j=0; j<final_operator[0].size(); j++){
            extraction_control_point[i][0] += final_operator[i][j]*control_point[j][0]; 
            extraction_control_point[i][1] += final_operator[i][j]*control_point[j][1]; 
        }
    }

    ofs.open("extracted_control_point.dat");
    for(int i=0; i<extraction_control_point.size(); i++){
        ofs << extraction_control_point[i][0] << " " << extraction_control_point[i][1] << endl;
    }
    ofs.close();
}