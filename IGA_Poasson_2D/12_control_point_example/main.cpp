    ...
    for(int dir=0; dir<2; dir++){
        for(int i=0; i<element_eta[eta_loop_num].size(); i++){//eta dir loop 3
            for(int j=0; j<element_xi[xi_loop_num].size(); j++){//xi dir loop 4
                    ...
                    for(int l=0; l<element_xi[xi_loop_num].size(); l++){ //xi dir loop 3
                        if(dir==0){
                            sum_dWdr += p_order_dNdr(knot_vector_xi,element_xi[xi_loop_num][l]+1,order_xi,gauss_1)\
                                *p_order_N(knot_vector_eta,element_eta[eta_loop_num][k]+1,order_eta,gauss_2)*control_weight[element[ic][k*3*l]];
                        ...
                        if(dir==1){
                            sum_dWdr +=  p_order_N(knot_vector_xi,element_xi[xi_loop_num][l]+1,order_xi,gauss_1)\
                                *p_order_dNdr(knot_vector_eta,element_eta[eta_loop_num][k]+1,order_eta,gauss_2)*control_weight[element[ic][k*3*l]];
                            sum_W += p_order_N(knot_vector_xi,element_xi[xi_loop_num][l]+1,order_xi,gauss_1)\
                                * p_order_N(knot_vector_eta,element_eta[eta_loop_num][k]+1,order_eta,gauss_2)*control_weight[element[ic][k*3*l]];
                        }
                    }
                ...
                dRdr[i*element_xi[xi_loop_num].size()+j][dir] = tmp_dRdr*control_weight[element[ic][i*3*j]];
            }
        }
    }
}
        ...
        for(double j=0; j<=1.0; j+=0.001){
            ...
            for(int k=0; k<4; k++){
                for(int l=0; l<3; l++){
                    ...
                    for(int m=0; m<4; m++){
                        for(int n=0; n<3; n++){
                            lower_sum += p_order_N(knot_vector_xi, n+1, order_xi, i) * p_order_N(knot_vector_eta, m+1, order_eta, j)*control_weight[m*3+n];
                        }
                    }
                    double tmp_R = p_order_N(knot_vector_xi, l+1, order_xi, i)*p_order_N(knot_vector_eta, k+1, order_eta, j)*control_weight[k*3+l];
                    tmp_R /= lower_sum;

                    r_sum_u += tmp_R * u[k*3+l];
                    r_sum_X += tmp_R * control_point[k*3+l][0];
                    r_sum_Y += tmp_R * control_point[k*3+l][1];
                }
            }
